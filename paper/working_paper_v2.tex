...
% Research highlights
\begin{highlights}
\item Z-score calibration and uncertainty-aware adaptive weighting, not geometric formulation, are the critical enablers of tri-modal coherence metrics — shown by ablation on two unrelated backbones.
...
\item A multi-backbone ensemble matches the strongest single backbone, showing that error decorrelation alone does not guarantee ensemble gains.
...
To establish ground truth, five independent raters evaluated 100 stratified samples (34 baseline, 33 wrong-image, 33 wrong-audio) via a web based interface implemented in Streamlit. The original 30-sample subset was additionally rated by three pilot raters; at eight raters, this subset reaches ICC(3,k) = 0.917, confirming the reliability of the protocol.
...
\item \textbf{Evaluation prompts.} We construct 91 unique scene descriptions spanning four environmental domains (nature, urban, water, mixed). Each evaluation triple pairs a description with a generated image and retrieved audio under one of three conditions: baseline (matched image with matched audio), wrong-image (cross domain mismatched image with matched audio), or wrong-audio (matched image with cross domain mismatched audio), yielding 100 evaluation triples (34 baseline / 33 wrong-image / 33 wrong-audio; by domain: nature 25, urban 30, water 10, mixed 35).

%91 scene descriptions across four domains (nature 25, urban 30, water 10, mixed 35), yielding 100 evaluation triples (34 / 33 / 33)


\item \textbf{Generative pipeline.} For each prompt, images are generated using  Stable Diffusion 1.5 \citep{rombach2022high} conditioned on the text prompt. Audio is retrieved via CLAP cosine similarity against an audio embedding index, selecting the best domain-compatible match. This setup reflects practical multimodal creation workflows where visual content is synthesized and audio is sourced from sound libraries.

\item \textbf{Perturbation protocol.} In the wrong-image condition, Stable Diffusion 1.5 generates an image from a different domain prompt (e.g., a nature prompt paired with an urban image). In the wrong-audio condition, a different domain audio clip is retrieved. This controlled perturbation design isolates incoherence to a single channel per condition.
...
\item \textbf{Hyperparameter selection.} Hyperparameters are selected via leave-one-out cross-validation (LOO-CV) over all 100 human-rated samples, an unbiased protocol in which each sample serves once as the held-out point. A stratified development/test split ($n=70$/$n=30$, by domain $\times$ condition, seed = 2024) is additionally maintained for supplementary held-out validation (Section~\ref{sec:eval-protocol}).
...
\item \textbf{cMSCI v3} (Section~\ref{sec:v3}): Multi-backbone ensemble combining v1 and v2 scores via a learned convex weight ($w_{v1}$ = 0.88), matching the accuracy of the stronger backbone while providing backbone redundancy. Designed for deployments requiring resilience across embedding providers.
\end{itemize}
...
where $s(\cdot,\cdot)$ denotes cosine similarity, $\mathbf{e}_t^{\text{CLIP}}$ and $\mathbf{e}_i^{\text{CLIP}}$ are CLIP text and image embeddings (ViT-B/32, 512-dimensional), $\mathbf{e}_t^{\text{CLAP}}$ and $\mathbf{e}_a^{\text{CLAP}}$ are CLAP text and audio embeddings (HTSAT-unfused, 512-dimensional), and $w_{ti} = w_{ta} = 0.50$.
...
This component is absent from cMSCI v2, where the unified embedding space enables direct image--audio comparison without projection. The v2 analysis (Section~\ref{sec:v2-exact-3d}) reveals that the image--audio channel in Gemini's unified space exhibits the lowest variance of the three channels and is optimally assigned zero weight ($w_{\mathrm{compl}}$ = 0.00, $w_{ia}$ = 0.00; see Section~\ref{sec:v2-exact-3d} for empirical analysis).
...
v1 requires two families of trained components that collectively address the limitations of the dual-space architecture (1,709,056 parameters total):
...
Both component families are trained on the same 10,255-pair dataset (OmniBench: 1,051 domain-matched triples; AudioCaps: 1,000 real audio-caption pairs; embedding-space augmentation: 8,204 pairs via Gaussian noise, dropout, and mixup) and require GPU resources for training but not for inference (all inference is forward-pass only). The scale of this training set matters: in the original 30-sample optimization, expanding it from 2,193 to 10,255 pairs shifted the optimized channel balance from image-dominated ($w_{ti} = 0.90$) to audio-inclusive ($w_{ti} = 0.30$), yielding a 6.5$\times$ improvement in audio incoherence detection sensitivity in that comparison; the current 100-sample optimization selects $w_{ti} = 0.15$ (Section~\ref{sec:v1-hyperparams}), consistent with this trend.
...
\midrule
\textbf{Total trainable} & \textbf{1.71M} & \textbf{Total trainable} & \textbf{0} \\
\bottomrule
\end{tabular}
\end{table}
...
However, empirical analysis reveals an important finding: in the Gemini unified space, the image-audio channel exhibits the lowest variance of the three channels ($\sigma_{ia} = 0.051$, compared to $\sigma_{ti} = 0.090$ and $\sigma_{ta} = 0.146$). The optimized image-audio weight is $w_{ia} = 0.00$, and the complementarity weight is $w_{\mathrm{compl}} = 0.00$. This indicates that while the unified space makes the IA channel \emph{computable}, it does not make it \emph{informative} for our evaluation data: the channel's limited variance means it contributes more noise than signal, a finding that has implications for the anatomy of unified embedding spaces (Section~\ref{sec:anatomic-unified}).
...
The deployed v2 configuration achieves $\rho = 0.544$ on the full sample (Table~\ref{tab:main-comparison}); LOO-CV yields $\rho = 0.577$, confirming that the configuration generalizes across held-out samples.
