...
from cmsci.coherence.gram_volume import (
    gram_volume_2d,
    gram_volume_3d,
    gram_volume_nd,
...
from cmsci.embeddings.aligned_embeddings import AlignedEmbedder
from cmsci.embeddings.similarity import cosine_similarity
...
class CalibratedCoherenceEngine:
    ...
    def __init__(
        ...
        if calibration_path and Path(calibration_path).exists():
            from cmsci.coherence.calibration import CalibrationStore
        ...
        if negative_bank_enabled:
            try:
                from cmsci.coherence.negative_bank import NegativeBank
        ...
        if exmcr_weights_path:
            from cmsci.embeddings.space_alignment import ExMCRProjector
        ...
        if bridge_path and Path(bridge_path).exists():
            from cmsci.embeddings.cross_space_bridge import CrossSpaceBridge
        ...
        if prob_clip_adapter_path and Path(prob_clip_adapter_path).exists():
            from cmsci.embeddings.probabilistic_adapter import ProbabilisticAdapter
